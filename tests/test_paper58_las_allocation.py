...
def test_allocate_can_prune_low_base_score_balanced_swaps():
    start = np.array([[1, 2]], dtype=np.int32)
    class_values = [1, 2]
    suitability = np.zeros((1, 2, 2), dtype=np.float32)
    suitability[0, 0, 1] = 0.04
    ...
    result = allocate_demand_constrained(
        ...
        balanced_swap_min_base_score=0.1,
    )
...
def test_allocate_keeps_high_base_score_balanced_swaps():
    start = np.array([[1, 2]], dtype=np.int32)
    class_values = [1, 2]
    suitability = np.zeros((1, 2, 2), dtype=np.float32)
...
def test_allocate_uses_pair_level_base_score_for_balanced_swaps():
    start = np.array([[1, 2]], dtype=np.int32)
    class_values = [1, 2]
    suitability = np.zeros((1, 2, 2), dtype=np.float32)
